...
description: Generate an interactive PR review walkthrough as an HTML page for any GitHub repository. Fetches PR data via gh API, categorizes files into core vs mechanical changes, adds reviewer annotations, and renders diffs with moved-code detection. Use when the user pastes a GitHub PR URL and asks for a review, walkthrough, or summary, or says "review this PR", "walk me through PR 42", or "make a canvas for this PR". Reviewer-facing companion to babysit-pr (which is the author/shipper side).
compatibility: Requires git, gh (GitHub CLI) authenticated, python3, and jq. Reads styles.css, renderer.js, template.html from this skill directory.
allowed-tools: Bash(gh:*) Bash(git:*) Bash(curl:*) Bash(python3:*) Bash(jq:*) Read Write
---
...
This is the **reviewer-facing** artifact — it explains an open PR to whoever reviews it. It complements
`babysit-pr`, which owns the **author/shipper** side (open → green → squash-merge). Works with any GitHub
repo. Explicit invocation only (`disable-model-invocation: true`) — call it with `/pr-review-canvas`.

## Redaction guardrail (read first)

This skill renders the PR body, review comments, and diffs into an HTML page. Before writing the final
HTML, scan the assembled body + patches for any secret or PII value the host project considers sensitive
(check `./AGENTS.md` or `./CLAUDE.md` for project-specific redaction rules) and **redact it to a
`[redacted]` form** if found. Never serve a page that embeds a secret or credential value, even locally.

## Workflow

### 1. Fetch PR data

**Auth:** `gh` must be authenticated (`gh auth status`); all PR data is read through it — no separate token
or API key. Resolve `{owner}/{repo}` with `$(gh repo view --json nameWithOwner -q .nameWithOwner)` from
the current checkout. Run these `gh api` calls in parallel:
